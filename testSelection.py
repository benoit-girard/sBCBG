...
def twoChannelCompetition(params={}, nbInNeurons=1000, nbSteps=11):
  ...
  for i2 in range(nbSteps):
    for i1 in range(nbSteps):
      ...
      print('* channel 2 act. level '+str(activityLevels[i2])+' (CSN: '+str(rate['CSN'][i2])+' Hz - PTN: '+str(rate['PTN'][i2])+' Hz)')
      ...
      nest.SetStatus(ActPop['CSN'][1],{'rate':rate['CSN'][i2]})
      nest.SetStatus(ActPop['PTN'][0],{'rate':rate['PTN'][i1]})
      nest.SetStatus(ActPop['PTN'][1],{'rate':rate['PTN'][i2]})
...
def main():
  ...
  twoChannelCompetition(params=params, nbInNeurons=500, nbSteps=5)
